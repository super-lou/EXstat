...
#'
#' @param data Input data format is a [tibble::tibble()] from the tibble package. It needs to have :
#' * Only one column of [base::Date] that are regularly spaced and unique for each time serie.
#' * If there is more than one time serie, at least one column needs to be of [base::character] for names of time series in order to identify them. If more than one column of identifier is given, they will all be used in order to identify a unique time serie.
#' * At least one column of [base::numeric] (or [base::logical]) on which the process of variable extraction will be perform. More numerical column can be leave but if they are useless, they will be suppressed.
#'
#' e.g.
#' ```
#' > data
...
#' ```
#' 
#' @param funct The function that you want to use for the process of variable extraction. More specificaly, it is possible to give a [base::list()] with several functions as element of that [base::list()] and the name that will be used for the extracted column as the names element of each function of that previously defined [base::list()]. A simple case will be `funct=mean` and a more complicated one `funct=list(QA=mean, QJXA=max)`.  
#' @param funct_args A [base::list()] of [base::list()] of named arguments needed for each functions provided through [funct]. This [base::list()] can be a simple [base::list()] if there is only one function given by [funct]. The argument can relate to a column name in order to specify on which numerical column the extraction will be perfom. For the simple example, `funct_args=list("Q_obs", na.rm=TRUE)` and for the more complex case `funct_args=list(list("Q_obs", na.rm=TRUE), list("Q_sim", na.rm=FALSE))`.
#' @param timeStep A [base::character] specifying the time step of the variable extraction process. Possible values are :
...
#' @param period A [vector][base::c()] of two [dates][base::Date] (or two unambiguous [base::character] that can be coerced to [dates][base::Date]) to restrict the period of analysis. As an example, it can be `c("1950-01-01", "2020-12-31")` to select data from the 1st January of 1950 to the end of December of 2020. The default option is `samplePeriod=NULL`, which considers all available data for each time serie.
#' @param isDate [base::logical]. If TRUE, [process_extration()] will convert the result of the application of [funct] to a day of the year. The aim is for example to give `funct=which.min` and if `isDate=TRUE`, the result will not be the indice of the minimum of the sample but the associated day of the year given by an [base::integer] (1 is the 1st of january).
#' @param NApct_lim [base::numeric]. The maximum percentage of missing values in each sample allowed. If this threshold is exceeded, the value associated to the current sample will be convert to NA.
#' @param NAyear_lim [base::numeric].The maximum number of continuous missing years allowed. If this threshold is exceeded, the time serie will be split in half around the problematic period and only the longest part will be used for the extraction process.
...
#' @param compress [base::logical]. If [timeStep] is set to `"month"` or `"season"`, should the function return a classical [tibble::tibble()] or a compressed [tibble::tibble()] ? Hence for these two [timeStep] a small number of time chunk will be present in the output (12 for `timeStep="month"` and by default 4 for `timeStep="season"`), it is possible to make a [tidyr::pivot_wider()] in order to have the time chunk indication in column and not anymore in line.
#'
#' e.g.
#' ```
...
#' ```
#' 
#' @param expand [base::logical]. If `TRUE`, expand the output [tibble::tibble()] as a [base::list()] of [tibble::tibble()] for each extracted variable.
#' @param rmNApct [base::logical]. Should the `NApct` column in the output that show the percentage of missing value should be remove ?
#' @param verbose [base::logical]. Should intermediate messages be printed during the execution of the function ?
#' 
#' @examples
#' # Date
...
process_extraction = function(data,
    ...
    # check Date column
    if (sum(sapply(data, lubridate::is.Date)) == 0 & !dev) {
    ...
    if (sum(sapply(data, is.character)) == 0 & !dev) {
        if (any(duplicated(
