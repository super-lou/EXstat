...
#' @description This process extracts a variable from time series (for example the yearly mean of time series). Extraction can be yearly, monthly, or realized each month of every year and sampled differently along years or months.
#'
#' @param data Input data format is a tibble of at least a column of date, a column of numeric value, and a character column for names of time series in order to identify them. Thus, it is possible to have a tibble with multiple time series which can be grouped by their names.
#' @param funct The function (for example `mean`) that you want to use. Arguments of the chosen function can be passed to this extraction process, and the function can be previously defined.
#' @param funct_args A list of arguments needed for the function provided through `funct`.
#' @param timeStep A character specifying the time step for sampling the data. Possible values are "year", "none", "yearday", "month", "year-month", "season", and "year-season".
...
#' @param period A vector of two dates to further restrict the period of analysis within the `data` tibble. The imposed date format is "YYYY-mm-dd". This parameter allows selecting a specific subset of the data.
#' @param isDate Logical. If TRUE, the function will assume that Date is a date and not a numeric value.
...
#' @param verbose Logical. Should intermediate messages be printed during the execution of the function?
#' @param ... Additional arguments needed for the function provided through `funct`.
#'
